  ...
  useEffect(() => {
    if (useWifi) {
      ...
      if ("getNetworkInformation" in navigator) {
        ...
        console.warn(
          "[Google Provider] WiFi scanning not available in browser"
        );
      }
    }
  }, [useWifi]);
  ...
  const getLocation = async () => {
    ...
    if (!currentApiKey) {
      const key = window.prompt(
        "Please enter your Google Geolocation API key:"
      );
      if (!key) {
    ...
    try {
      ...
      if (useCellular && networkInfo.mcc && networkInfo.mnc) {
        if (!/^\d+$/.test(networkInfo.mcc)) {
          throw new LocationError(
            ...
            "Mobile Country Code must be numeric"
          );
        }
        if (!/^\d+$/.test(networkInfo.mnc)) {
          throw new LocationError(
            ...
            "Mobile Network Code must be numeric"
          );
        }
      ...
      if (useWifi) {
        if (wifiNetworks.length >= 2) {
          ...
          console.warn(
            "[Google Provider] Insufficient WiFi access points available"
          );
      ...
      const response = await fetch(
        `https://www.googleapis.com/geolocation/v1/geolocate?key=${currentApiKey}`,
        {
          method: "POST",
      ...
      if (!response.ok) {
        ...
        if (
          ...
          const newKey = window.prompt(
            "Your API key was invalid. Please enter a valid Google Geolocation API key:"
          );
          if (newKey) {
            setApiKey(newKey);
